...
public:
    ...
    template<typename T> void write(T&& p, size_t n = 1) { db.write(reinterpret_cast<char const*>(&p), sizeof(T) * n); }

    template<typename T> void read(T& p, size_t n = 1) { db.read(reinterpret_cast<char*>(&p), sizeof(T) * n); }

    fstream* operator->() { return &db; }
};
